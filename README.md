...
private Contour Gateway that remains user-owned. The verification proves
ai-extensions PR
[#672](https://github.com/radius-project/ai-extensions/pull/672) routes through
...
Until ai-extensions PR #672 merges and the Radius Canvas ships that source,
do not use the Canvas Deploy or Delete controls: regeneration from an older
Canvas build would overwrite these pre-merge workflow pins. Use the
`setup.sh deploy` and `setup.sh delete` commands below. After that merge gate,
the Canvas controls may be used once their generated workflows reference PR
#672 or a descendant commit.
...
Every first-party extension action in the Azure deploy and delete workflows,
including `manage-routes-gateway`, is pinned to the PR #672 source commit
`radius-project/ai-extensions@246f09f3b6313c705fb38d5ae7276356a5c9aba8`.
